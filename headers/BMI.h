...
#include <math.h> // needed to perform area
...
BMI::BMI(double height, double weight)
{
    ...
    // check if we've been given an invalid argument - weight less then zero
            if (weight < 0){
                // throw an exception
                throw invalid_argument("weight  must be greater that or equal to zero.");
            }
    this->height = height;
    this->weight = weight;
}
